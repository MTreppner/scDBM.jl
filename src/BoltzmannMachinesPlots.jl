...
function getvalue(monitor::Monitor, evaluation::String, epoch::Int)
   itemidx = findfirst((item -> item.evaluation == evaluation && item.epoch == epoch), monitor)
   monitor[itemidx].value
end

function extractevaluationdata(monitor::Monitor, evaluation::String)
...
function extractaisdata(monitor::Monitor, evaluation::String, sdrange::Float64)
   ...
   if sdrange != 0.0
      ...
      for epoch in epochs
         sd = getvalue(monitor, monitoraisstandarddeviation, epoch)
         logr = getvalue(monitor, monitoraislogr, epoch)

         # log(Z) is subtracted from logproblowerbound, so overstimating log(Z)
         # means underestimating the log probability
         bottom, top = aisprecision(logr, sd, sdrange)
...
"
function plotestimatedprob(monitor::Monitor, evaluationkey::String;
      sdrange::Float64 = 0.0)
...
plottitledict = Dict(
      monitorreconstructionerror => "Mean reconstruction error",
      monitorlogproblowerbound => "Average lower bound of log probability",
      monitorloglikelihood => "Average log-likelihood",
      monitormeandiff => "L²-difference between means \nof generated and original data",
      monitorexactloglikelihood => "Exact log-likelihood",
      monitorweightsnorm => "L²-norm of weights",
      monitorsd => "Standard deviation parameters of visible units",
      monitorcordiff => "L²-difference between correlation matrices \nof generated and original data",
      monitorfreeenergy => "Free energy")
...
"""
function plotevaluation(monitor::Monitor,
      evaluationkey::String = monitor[1].evaluation;
      sdrange::Float64 = 2.0, changetitle::Function = identity)

   if evaluationkey in [monitorloglikelihood; monitorlogproblowerbound]
...
"""
function scatterhidden(bm::AbstractBM, x::Matrix{Float64};
      hiddennodes::Tuple{Int,Int} = (1,2),
      labels = Vector{String}())

   function hiddenactivations(rbm::AbstractRBM, x)
      logit.(hiddenpotential(rbm, x))
   end

   function hiddenactivations(rbm::AbstractXBernoulliRBM, x)
      hiddeninput(rbm, x)
   end

   function hiddenactivations(dbm::MultimodalDBM, x)
      logit.(meanfield(dbm, x)[end])
   end
...
function crossvalidationcurve(monitor::Monitor,
         evaluation::String = "")
